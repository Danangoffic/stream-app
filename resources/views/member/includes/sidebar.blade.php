...
<aside class="fixed z-50 w-[360px] bg-stream-dark">
    <div class="flex flex-col p-12 border-r border-softpur overflow-y-auto h-screen">
        <a href="{{ route('member.dashboard') }}">
            <img src="{{ asset('user/assets/images/stream.svg') }}" alt="">
        </a>
        <div class="links flex flex-col mt-16 gap-2">
            <a href="{{ route('member.dashboard') }}"
                class="side-link @if (\Route::is('member.dashboard') || \Route::is('member.movie.detail')) active @else @endif">
            ...
            <a href="{{ route('member.subscription') }}"
                class="side-link @if (\Route::is('member.subscription')) active @else @endif">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
